...
PREEMPT_ETA_WINDOW_S = 10.0    # only hold TL preemption while EV is near junction
RELEASE_DIST_M       = 95.0    # release held cars once EVs are far from that zone
...
class V2VController:
    ...
    def _preempt_signals_early(self, cur):
        ...
        # Do not keep the whole intersection artificially locked forever.
        # If all EVs are far away, restore normal TL program immediately.
        if best_eta > PREEMPT_ETA_WINDOW_S:
            if self.tl_phase_locked:
    ...
    def _emergency_corridor(self, cur):
        ...
        self._release_passed_zone_vehicles(emg_info, cur)
    ...
    def _release_passed_zone_vehicles(self, emg_info, cur):
        """Release previously held vehicles once EVs are no longer nearby."""
        held = (self.pullover_vids | self.waiting_vids)
        for vid in held:
            if vid not in cur or vid in self.restored_vids:
                continue
            if not self._veh_known(vid):
                continue
            try:
            ...
            min_d = float('inf')
            for emg_id, (ex, ey, _lanes, _edges) in emg_info.items():
                d = math.sqrt((vx - ex) ** 2 + (vy - ey) ** 2)
                if d < min_d:
                    min_d = d

            if min_d > RELEASE_DIST_M:
                try:
                    traci.vehicle.setSpeedMode(vid, 31)
                    traci.vehicle.setLaneChangeMode(vid, 1621)
                    traci.vehicle.setSpeed(vid, -1)
